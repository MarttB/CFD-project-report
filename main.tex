...
\newcommand{\fourthauthor}{Leo De Luca, 10694949}
...
\label{sec:eqs}
%This section describes how the numerical simulations are carried out. It provides details and justifications about the modeling choices, the meshes, and the numerical schemes. Do not include the \texttt{.config} file or parts of it. Describe the options you consider important using words, rather than inserting option strings from the file.
...
%f_\text{bump}(x,y) = \frac{\sqrt{x^2 \tan^2(\delta) + c}}{\left(\frac{1}{\cos(\arctan(\frac{y}{K}))}\right)^2} \sin(x) \sin\left(\frac{y + \pi}{2}\right)
\label{eq:BUMP GEOM}
\end{equation}
...
\begin{table}[H]
    \centering
    \scalebox{0.9}{\begin{tabularx}{\textwidth}{*{6}{| >     {\centering\arraybackslash}X}|}
    \hline
        \textbf{BUMP} & $\delta$ & c & K & A & B \\
        \hline
         & $\pi/15$ & 0 & 1.0 & $\sin{x}$ & $\sin\left(\frac{y + \pi}{2}\right)$ \\
        \hline
    \end{tabularx}}
